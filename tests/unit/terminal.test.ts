...
describe('Terminal session resiliency', () => {
  ...
  it('still sends prompt nudge when pre-ready queued input is only carriage return', async () => {
    ...
    const stdinFrames = socket.sent.filter((frame) => frame === '0:1:\r');
  ...
  it('reapplies latest resize before forwarding stdin', async () => {
    const socket = new mockedWs.MockWebSocket('wss://pve.local/ws');
    const session = new TerminalSession(
      async () => ({
        socket: socket as never,
    ...
    const resizeFrames = socket.sent.filter((frame) => frame === '1:120:40:');
    const stdinFrames = socket.sent.filter((frame) => frame === '0:3:abc');
    expect(resizeFrames.length).toBe(2);
    expect(stdinFrames.length).toBe(1);
    expect(socket.sent.indexOf('1:120:40:')).toBeLessThan(socket.sent.lastIndexOf('0:3:abc'));
  });
  ...
  it('accepts structured json resize and input frames', async () => {
    ...
    const resizeFrames = socket.sent.filter((frame) => frame === '1:90:33:');
    const inputFrames = socket.sent.filter((frame) => frame === '0:14:R:not-a-resize');
  ...
  it('applies resize metadata embedded in structured input frames', async () => {
    ...
    const resizeFrames = socket.sent.filter((frame) => frame === '1:132:44:');
    const inputFrames = socket.sent.filter((frame) => frame === '0:1:x');

    expect(resizeFrames.length).toBe(2);
    expect(inputFrames.length).toBe(1);
    expect(socket.sent.indexOf('1:132:44:')).toBeLessThan(socket.sent.indexOf('0:1:x'));
    expect(socket.sent.lastIndexOf('1:132:44:')).toBeLessThan(socket.sent.indexOf('0:1:x'));
  });
});
